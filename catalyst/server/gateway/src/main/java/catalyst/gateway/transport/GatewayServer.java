...
public final class GatewayServer {
    ...
    private final Map<String, BackendClient> dynamicWorldClients = new ConcurrentHashMap<>();
    ...
    public GatewayServer(GatewayProperties props, TlsProperties tlsProps) {
        ...
        for (Map.Entry<ServiceType, GatewayProperties.BackendConfig> entry : props.getBackends().entrySet()) {
            ...
            log.info("Configuring static backend client for '{}' connecting to {}:{}", type, config.host(), config.port());
    ...
    public void start() throws Exception {
        ...
        bound = true;
        log.info("Gateway Edge Server bound on UDP port {}", props.getPort());
    }
