package catalyst.server;

import catalyst.common.concurrency.TaskScheduler;
import catalyst.server.config.ServerProperties;
import catalyst.server.dispatch.MessageDispatcher;
import catalyst.server.handler.LoginHandler;
import catalyst.server.repository.SessionRepository;
import catalyst.server.transport.QuicServerTransport;
...
public class ServerApplication {
    ...
    public void onStartup(StartupEvent event) throws Exception {
        ...
        transport.start();
        log.info("Catalyst server listening on UDP port {} (QUIC)", props.getPort());
